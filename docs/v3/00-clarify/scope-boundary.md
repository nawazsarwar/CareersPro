...
| **M17** | **Reservation & Roster Engine** | **v1-partial** | Roster registers, backlog tracking, statutory reporting. **Built as an externally configurable policy plug-in with per-advertisement effective dating — never hard-coded.** No substantive reservation rules exist in either source document (DOC-003), and AMU's minority-institution status is live litigation (*AMU v. Naresh Agarwal*, 7-judge bench, Nov 2024, remitted). **v1 ships the mechanism; the policy data lands when DOC-003 and OQ-013 close.** |
